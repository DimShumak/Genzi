    ...
    $i = 0;
        ...
        if(count($arResult['DISPLAY_PROPERTIES']['GALLERY']['VALUE']) != 1) {
            foreach($arResult['DISPLAY_PROPERTIES']['GALLERY']['FILE_VALUE'] as $arItem){
                $arResult['PROPERTIES']['GALLERY']['FILE_VALUE'][$i] = \CFile::ResizeImageGet($arItem, array('width' => 1000, 'height' => 800), BX_RESIZE_IMAGE_EXACT, true);
                $arResult['PROPERTIES']['GALLERY']['FILE_VALUE'][$i]['webp_src'] = Util::getInstance()->makeWebp($arResult['PROPERTIES']['GALLERY']['FILE_VALUE'][$i]['src']);
                $i++;
            }
        }
        else{
            $arResult['PROPERTIES']['GALLERY']['FILE_VALUE'][0] = \CFile::ResizeImageGet($arResult['DISPLAY_PROPERTIES']['GALLERY']['FILE_VALUE'], array('width' => 800, 'height' => 800), BX_RESIZE_IMAGE_EXACT, true);
