	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		$arResult = array_merge(
			...
			array(
				...
				"FORM_HEADER" => sprintf( // form header (<form> tag and hidden inputs)
					"<form name=\"%s\" action=\"%s\" method=\"%s\" class=\"%s\" enctype=\"multipart/form-data\">",
					$arResult["arForm"]["SID"], POST_FORM_ACTION_URI, "POST", $arParams['FROM_CLASS']
		...
		*/
		...
		foreach ($arResult["arQuestions"] as $key => $arQuestion)
		{
            ...
            $arResult["QUESTIONS"][$FIELD_SID]['CAPTION'] = explode("|", $arResult["QUESTIONS"][$FIELD_SID]['CAPTION'])[0];
			...
			if (is_array($arResult["arAnswers"][$FIELD_SID]))
			{
				...
				foreach ($arResult["arAnswers"][$FIELD_SID] as $key => $arAnswer)
				{
                    ...
                    foreach($arResult['arQuestions'] as $arFiled){
                        if($arAnswer['FIELD_ID'] == $arFiled['ID']){
                            $arFiledLabel = @explode('|', $arFiled['TITLE'])[0];
                            $arFiledClass = @explode('|', $arFiled['TITLE'])[1];
                            $required = $arFiled['REQUIRED'] == 'Y' ? 'required ' : '';

							$parseLabel = @explode('#', $arFiledLabel);
							if(LANGUAGE_ID == 'en' && count($parseLabel) >= 2) {
								$arFiledLabel = $parseLabel[1];
							} elseif(LANGUAGE_ID == 'en' && count($parseLabel) < 2){
								$arFiledLabel = $parseLabel[0];
							} elseif(LANGUAGE_ID == 'ru' && count($parseLabel) >= 2){
								$arFiledLabel = $parseLabel[0];
							}
							
                        }
                    }
                    $arAnswer["FIELD_PARAM"] = $arAnswer["FIELD_PARAM"] . "placeholder=\"$arFiledLabel\"";
                    $arAnswer["FIELD_PARAM"] = $arAnswer["FIELD_PARAM"] . "$required";
                    $arAnswer["FIELD_PARAM"] = $arAnswer["FIELD_PARAM"] . "class=\"input-control $arFiledClass\"";
					...
					switch ($arAnswer["FIELD_TYPE"])
					{
						...
						case "dropdown":
							if ($show_dropdown!="Y")
							{
                                ...
                                array_unshift($arResult["arDropDown"][$FIELD_SID]['reference'], $arFiledLabel);
                                array_unshift($arResult["arDropDown"][$FIELD_SID]['reference_id'], "9999");
                                array_unshift($arResult["arDropDown"][$FIELD_SID]['param'], '');
						...
						case "file":
                            ...
                            $arAnswer['FIELD_PARAM'] .= "id=\"input-control-file_" . $arAnswer['ID'] ."\"";
                            $arAnswer['FIELD_PARAM'] .= "data-label=\"$arFiledLabel\"";
                            $res .= "<div class=\"input-control-file\"><label for=\"input-control-file_" . $arAnswer['ID'] . "\">".$arFiledLabel."</label>";
							$res .= CForm::GetFileField(
								$arAnswer["ID"],
								$arAnswer["FIELD_WIDTH"],
								"FILE",
        ...
        // $APPLICATION->AddChainItem($arResult['arForm']['NAME']);
